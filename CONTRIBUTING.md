...
- **immuni_analytics**. This contains the logic of the specific backend service.
tests. This contains the unit tests.

The *immuni_analytics* folder is grouped by concerns. Specifically, as follows:
...
- **tasks**. This contains the pieces of functionality to be executed asynchronously, either in response to a specific API request or to a predefined schedule (e.g., data erasure).

### Microservices
The backend service logic runs microservices, each one identified by a dedicated file within the *immuni_analytics* folder. Specifically, as follows:

- **celery.py**. This contains the celery_app object, representing the [Celery](https://pypi.org/project/celery/) distributed task management microservice. 
